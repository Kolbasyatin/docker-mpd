...
RUN useradd mpd && mkdir -p /run/mpd && mkdir -p /var/lib/mpd && touch /var/lib/mpd/{playlists,tag_cache,mpd.log,state,sticker.sql} && chown mpd:mpd -R /var/lib/mpd && chown mpd:mpd -R /run/mpd  
